...
class ValueFunctionDQN_TRAIN_DROPOUT:
    ...
    def model(self, data):
        ...
        #dropout applied to hidden layer 1
        drop_out = tf.nn.dropout(hidden1, self.dropout_keep_prob)

        logits2 = tf.matmul(hidden1, self.l2_weights) + self.l2_biases
        hidden2 = tf.nn.relu(logits2)  # Define units of layer
        #dropout applied to hidden layer 2
        drop_out_2 = tf.nn.dropout(hidden2, self.dropout_keep_prob)
        out_layer = tf.matmul(drop_out_2, self.l3_weights) + self.l3_biases
    ...
    def train(self, states, targets):
        ...
        feed_dict = {self.train_data: states, self.train_targets: targets, self.dropout_keep_prob : 0.5}
...
class ValueFunctionDQN_TEST_TRAIN_DROPOUT:
    ...
    def predict(self, states):
        ...
        feed_dict = {self.x: states, self.dropout_keep_prob : 0.5}
        q = self.session.run(self.test_prediction, feed_dict=feed_dict)
        return q
